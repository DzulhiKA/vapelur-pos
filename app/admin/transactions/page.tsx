...
export default function TransactionsPage() {
  ...
  const filterLabel = () => {
    const parts: string[] = []
    if (filter.date) parts.push(`Tanggal: ${formatDateShort(filter.date + 'T00:00:00')}`)
    if (filter.method) parts.push(`Metode: ${filter.method === 'cash' ? 'Tunai' : 'QRIS'}`)
  ...
  return (
    <>
        ...
        <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '14px', margin: 0 }}>
              {transactions.length} transaksi ditemukan · Total:{' '}
              <span style={{ color: 'var(--accent)', fontWeight: '700' }}>{formatRupiah(totalRevenue)}</span>
            </p>
          </div>
          ...
          <button
            ...
            style={{
              ...
              gap: '8px',
              padding: '10px 20px',
              background: transactions.length === 0 ? 'var(--bg-hover)' : 'rgba(34,197,94,0.1)',
              border: `1px solid ${transactions.length === 0 ? 'var(--border)' : 'rgba(34,197,94,0.4)'}`,
              borderRadius: '10px',
      ...
      {/* ========== PRINT-ONLY REPORT (always in DOM, hidden on screen) ========== */}
      <div
        id="transaction-print-report"
        style={{ display: 'none' }}
        aria-hidden="true"
      >
        {/* Report Header */}
        <div style={{ marginBottom: '16px', borderBottom: '2px solid #333', paddingBottom: '12px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <h1 style={{ margin: '0 0 4px', fontSize: '20px', fontWeight: '800', color: '#111' }}>
                💨 Laporan Transaksi
              </h1>
        ...
        <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
          <div style={{ padding: '10px 16px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '6px', flex: 1 }}>
            <p style={{ margin: '0 0 2px', fontSize: '10px', color: '#15803d', fontWeight: '600', textTransform: 'uppercase' }}>Total Pendapatan</p>
            <p style={{ margin: 0, fontSize: '16px', fontWeight: '800', color: '#15803d' }}>{formatRupiah(totalRevenue)}</p>
          </div>
          <div style={{ padding: '10px 16px', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '6px' }}>
            <p style={{ margin: '0 0 2px', fontSize: '10px', color: '#1d4ed8', fontWeight: '600', textTransform: 'uppercase' }}>Selesai</p>
            <p style={{ margin: 0, fontSize: '16px', fontWeight: '800', color: '#1d4ed8' }}>{completedCount}</p>
          </div>
          <div style={{ padding: '10px 16px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '6px' }}>
            <p style={{ margin: '0 0 2px', fontSize: '10px', color: '#dc2626', fontWeight: '600', textTransform: 'uppercase' }}>Dibatalkan</p>
            <p style={{ margin: 0, fontSize: '16px', fontWeight: '800', color: '#dc2626' }}>{cancelledCount}</p>
          </div>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
          <thead>
            <tr style={{ background: '#f3f4f6' }}>
              <th style={{ padding: '8px 10px', textAlign: 'left', fontWeight: '700', color: '#374151', borderBottom: '2px solid #d1d5db', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>No</th>
              <th style={{ padding: '8px 10px', textAlign: 'left', fontWeight: '700', color: '#374151', borderBottom: '2px solid #d1d5db', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Kode Transaksi</th>
              <th style={{ padding: '8px 10px', textAlign: 'left', fontWeight: '700', color: '#374151', borderBottom: '2px solid #d1d5db', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Kasir</th>
              <th style={{ padding: '8px 10px', textAlign: 'center', fontWeight: '700', color: '#374151', borderBottom: '2px solid #d1d5db', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Item</th>
              <th style={{ padding: '8px 10px', textAlign: 'left', fontWeight: '700', color: '#374151', borderBottom: '2px solid #d1d5db', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Metode</th>
              <th style={{ padding: '8px 10px', textAlign: 'right', fontWeight: '700', color: '#374151', borderBottom: '2px solid #d1d5db', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Subtotal</th>
              <th style={{ padding: '8px 10px', textAlign: 'right', fontWeight: '700', color: '#374151', borderBottom: '2px solid #d1d5db', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Diskon</th>
              <th style={{ padding: '8px 10px', textAlign: 'right', fontWeight: '700', color: '#374151', borderBottom: '2px solid #d1d5db', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Total</th>
              <th style={{ padding: '8px 10px', textAlign: 'center', fontWeight: '700', color: '#374151', borderBottom: '2px solid #d1d5db', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Status</th>
              <th style={{ padding: '8px 10px', textAlign: 'left', fontWeight: '700', color: '#374151', borderBottom: '2px solid #d1d5db', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Waktu</th>
            </tr>
          </thead>
          <tbody>
            {transactions.map((trx, idx) => (
              <tr
                ...
                style={{
                  ...
                  opacity: trx.status === 'cancelled' ? 0.6 : 1,
                }}
              >
                <td style={{ padding: '7px 10px', color: '#6b7280' }}>{idx + 1}</td>
                <td style={{ padding: '7px 10px', fontFamily: 'monospace', fontSize: '10px', color: '#374151' }}>
                  {trx.transaction_code}
                </td>
                <td style={{ padding: '7px 10px', color: '#374151' }}>{trx.cashier_name || '-'}</td>
                <td style={{ padding: '7px 10px', textAlign: 'center', color: '#374151' }}>
                  {trx.transaction_items?.length || 0}
                </td>
                <td style={{ padding: '7px 10px', color: '#374151', textTransform: 'uppercase', fontSize: '10px', fontWeight: '600' }}>
                  {trx.payment_method === 'cash' ? 'Tunai' : 'QRIS'}
                </td>
                <td style={{ padding: '7px 10px', textAlign: 'right', color: '#374151' }}>
                  {formatRupiah(trx.subtotal)}
                </td>
                <td style={{ padding: '7px 10px', textAlign: 'right', color: trx.discount > 0 ? '#dc2626' : '#9ca3af' }}>
                  {trx.discount > 0 ? `-${formatRupiah(trx.discount)}` : '-'}
                </td>
                ...
                <td style={{ padding: '7px 10px', textAlign: 'center' }}>
                  <span style={{
                    ...
                    fontSize: '10px',
                    fontWeight: '600',
                    background: trx.status === 'completed' ? '#dcfce7' : '#fee2e2',
                    color: trx.status === 'completed' ? '#15803d' : '#dc2626',
                  }}>
                    {trx.status === 'completed' ? 'Selesai' : 'Dibatalkan'}
                  </span>
                </td>
                <td style={{ padding: '7px 10px', fontSize: '10px', color: '#6b7280', whiteSpace: 'nowrap' }}>
                  {formatDate(trx.created_at)}
                </td>
              </tr>
            ))}
          ...
          <tfoot>
            <tr style={{ borderTop: '2px solid #333', background: '#f9fafb' }}>
              <td colSpan={7} style={{ padding: '8px 10px', fontWeight: '700', fontSize: '12px', color: '#111', textAlign: 'right' }}>
                TOTAL PENDAPATAN (transaksi selesai):
              </td>
              <td style={{ padding: '8px 10px', fontWeight: '800', fontSize: '13px', color: '#15803d', textAlign: 'right' }}>
        ...
        {/* Report Footer */}
        <div style={{ marginTop: '20px', paddingTop: '10px', borderTop: '1px solid #d1d5db', textAlign: 'center' }}>
          <p style={{ margin: 0, fontSize: '10px', color: '#9ca3af' }}>
            Laporan ini digenerate otomatis oleh sistem Vapelur POS
          </p>
        </div>
      </div>
    </>
